...
def load_reference_keys(scope: str) -> tuple[set[str], set[str], set[str]]:
    """对照 909–911 样本库：scope=all 为三天全量（纯新品），train 仅训练折。"""
    ds = pd.read_csv(ART / "dataset.csv", encoding="utf-8-sig", dtype={"商品ID": str})
    if scope == "train":
        sub = ds.loc[ds["split"] == "train"]
    else:
        sub = ds
    ref_ids = set(sub["商品ID"].astype(str))
    ref_urls = set(sub["main_url"].dropna().astype(str)) - {""}
    ref_titles = set(sub["标题"].astype(str).str.strip()) - {""}
    return ref_ids, ref_urls, ref_titles
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=("all", "train"),
        default="all",
        help="去重对照库：all=909–911 全量，train=仅训练折",
    )
    args = ap.parse_args()
    inp = Path(args.input)
    ...
    ref_ids, ref_urls, ref_titles = load_reference_keys(args.ref)
    print(
        f"[ref={args.ref}] 对照库: ID {len(ref_ids)} · 主图URL {len(ref_urls)} · 标题 {len(ref_titles)}"
    )
    ...
    df, tstats = drop_vs_train(df, ref_ids, ref_urls, ref_titles)
    n1b = len(df)
    ...
    print(
        f"原始 {n0} → 本表去重 {n1} → 剔除对照库重合 {n1 - n1b} "
