...
from render_html import render, render_share, rows_from_df  # noqa: E402
...
def main() -> None:
    ...
    cache_pair = load_pair(art_dir) if art_dir else None
    t0 = time.perf_counter()
    if cache_pair:
    ...
    t0 = time.perf_counter()
    html_path = out_dir / f"{stem}_screener.html"
    row_data = rows_from_df(export)
    render(row_data, f"选品筛子 · {inp.name}", html_path)
    share_path = out_dir / f"{stem}_分享.html"
    render_share(row_data, f"选品 Top5% · {inp.name}", share_path, top_pct=5.0)
    t_html = time.perf_counter() - t0

    print(
        f"[run] 耗时 读表{t_read:.1f}s 下载{t_dl:.1f}s 图{t_img:.1f}s 文{t_txt:.1f}s "
        f"预测{t_pred:.1f}s 违禁{t_word:.1f}s HTML{t_html:.1f}s 合计{time.perf_counter()-t_all:.1f}s"
    )
    print(f"[run] CSV {csv_path}")
    print(f"[run] HTML {html_path}")
    print(f"[run] 分享页 {share_path}")
