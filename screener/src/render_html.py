...
def render_share(rows: list[dict], title: str, out: Path, top_pct: float = 5.0) -> None:
    ...
    if top_pct < 100:
        ...
        rows = rows[:n]
        top_n = n
    blocked_n = sum(1 for r in rows if r.get("banned") or is_flat_print_product(r))
    rows = [r for r in rows if not r.get("banned") and not is_flat_print_product(r)]
    cards = []
    for r in rows:
        ...
        cards.append(
            f"""<article class="card" data-id="{_esc(r.get('id', ''))}" data-price="{price:.4f}" data-cate="{cate}" data-title="{title_s}">
  ...
  <div class="body">
    ...
    cat_options = "\n".join(f'<option value="{_esc(c)}">{_esc(c)}</option>' for c in cats)
    page = f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8"/>
...
<header class="hdr">
  <h1>{safe_title}</h1>
  <p>模型 Top {top_pct:g}% 原始 {top_n} 条 · 已自动剔除违禁/2D平面类 {blocked_n} 条 · 本页 {n_total} 条 · 主图 https 直链（需联网）</p>
  <div class="bar">
...
(function() {{
  ...
  function apply() {{
    ...
    cards.forEach(el => {{
      ...
      let ok = true;
      if (isDropped && !showDropped) ok = false;
      if (!isNaN(pmin) && p < pmin) ok = false;
      if (!isNaN(pmax) && p > pmax) ok = false;
      if (kw && title.indexOf(kw) < 0) ok = false;
      if (cate && cat !== cate) ok = false;
