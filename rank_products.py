...
SEED = 42
# cat_l1 在全表为常量「家居厨房用品」，不作为特征
CAT_COLS = ("cat_l2", "source")
NUM_TAB_COLS = (
    "log_price",
    "has_video",
    "n_gallery",
...
async def download_urls(urls: list[str], concurrency: int = 200) -> dict[str, bool]:
    ...
    return {url: ok for url, ok, _ in results}
...
def stage_embed() -> None:
    ...
    img_path = ART / "img.npz"
    txt_path = ART / "txt.npz"
    ids = df["商品ID"].tolist()

    if img_path.exists():
        z = np.load(img_path, allow_pickle=True)
        cache_ids = [str(i) for i in z["ids"]]
        if cache_ids == ids:
            X_img = z["X"]
            print(f"[embed] 读取缓存 {img_path}")
        elif set(cache_ids) == set(ids):
            df = df.set_index("商品ID").loc[cache_ids].reset_index()
            ids = cache_ids
            X_img = z["X"]
            print(f"[embed] 读取缓存 {img_path}（按缓存顺序对齐）")
        else:
            X_img = None
    else:
        X_img = None
    ...
    if X_img is None:
        paths = [url_to_path(u) for u in df["main_url"]]
        model, tfm = build_resnet()
        dev = device()
        print(f"[embed] ResNet18 device={dev} batch={img_batch_size()}")
        ...
        if len(ok_idx) < len(df):
            ...
            raw = raw
            ids = df["商品ID"].tolist()
            print(f"[embed] 解码失败再丢弃 {len(paths) - len(ok_idx)} 行")
        X_img = l2_normalize(raw)
        np.savez_compressed(img_path, ids=np.array(ids, dtype=object), X=X_img)
        print(f"[embed] 写入 {img_path} shape={X_img.shape}")

    if txt_path.exists():
        z = np.load(txt_path, allow_pickle=True)
        if list(z["ids"]) == ids:
            print(f"[embed] 读取缓存 {txt_path}")
        else:
            texts = df["标题"].astype(str).tolist()
            print("[embed] BGE 文本向量…")
            X_txt = embed_texts(texts, batch=32)
            np.savez_compressed(txt_path, ids=np.array(ids, dtype=object), X=X_txt)
    ...
    for name in EVAL_GROUPS:
        ...
        if name == "full":
            with open(ART / "lgbm_full.pkl", "wb") as f:
                pickle.dump(model, f)
            model.booster_.save_model(str(ART / "lgbm_full.txt"))
