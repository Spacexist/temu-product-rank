...
TRAIN_SOURCES = ("909.xlsx", "910.csv")
TEST_SOURCE = "911.csv"
CAT_COLS = ("cat_l2",)
...
def load_aligned() -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    z_img = np.load(ART / "img.npz", allow_pickle=True)
    z_txt = np.load(ART / "txt.npz", allow_pickle=True)
...
def verdict(full_lift_gt0: float) -> str:
    ratio = full_lift_gt0 / REF_SHOP_FULL["prec_sale_gt0_lift_vs_base"]
    if ratio >= 0.8:
        return "A", ratio
    if ratio >= 0.5:
        return "B", ratio
...
def main() -> None:
    ...
    print(f"[temporal] train(909+910) {len(train_all)} · test(911) {len(test_all)}")
    ...
    print(f"[temporal] y winsorize p99.5 (train 909+910) = {y_cap:.1f}")
    ...
    grade, ratio = verdict(full_row["lift_gt0"])
...
def write_report(data: dict) -> None:
    ...
    lines = [
        ...
        "## 设定",
        "",
        f"- train：`909.xlsx` + `910.csv`（拟合 {data['meta']['n_train_fit']}，valid {data['meta']['n_valid']}）",
        f"- test：`911.csv` 清洗后 **{data['meta']['n_test_after_dedupe']}** 行（去重剔除 {data['meta']['dedupe_removed']}）",
        f"- 特征：full = 表 + ResNet + BGE，**`CAT_COLS` 仅 `cat_l2`（无 `source`）**",
        f"- y：`log1p(clip(销量, 0, {data['meta']['y_cap_p995_train']:.1f}))`，p99.5 仅在 909+910 train 上算",
    ...
    lines += [
        ...
        f"- test 清洗后 **{n_test}** 行（TASK 初稿写的 ~2.9k 与当前缓存对齐后的 911 规模不符，以本次为准）。",
        "- 未重训/改动 `rank_products.py`；未覆盖主图缺失 SKU。",
        "",
    ]
    (ART / "temporal_validation.md").write_text("\n".join(lines), encoding="utf-8")
